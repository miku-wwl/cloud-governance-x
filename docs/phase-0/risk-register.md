...
| RISK-0002 | 无业务 tenant 隔离 | Security/Data | Day 20 已接受 ADR-0003；Day 21 已增加 tenancy schema 和跨 tenant 外键；Day 22 已增加 HTTP Membership 验证和后台显式 TenantContext；当前三张核心业务表仍无 `tenant_id` 和 Repository tenant predicate；GAP-002 | 引入第二组织或账号边界 | 跨组织数据越界和错误操作 | Medium | Critical | Critical | Platform Architect | Avoid | 阶段 2～3 | tenant escape、IDOR、RLS 与后台上下文测试 | Open | N/A |
