...
| RISK-0002 | 无业务 tenant 隔离 | Security/Data | Day 23 已为资源、成本、ETL 增加 tenant_id、Repository tenant predicate、tenant 唯一键、CloudAccount 归属外键和真实 PostgreSQL A/B/IDOR 测试；历史行仍为 NULL，OIDC/RBAC/RLS 未完成；GAP-002 | 引入第二组织或账号边界 | 跨组织数据越界和错误操作 | Low | Critical | High | Platform Architect | Avoid | 阶段 2～3 | tenant escape、IDOR、RLS 与后台上下文测试 | Mitigated | Day 24～30 完成剩余控制 |
