...
| RISK-0002 | 无业务 tenant 隔离 | Security/Data | Day 23 已隔离新读写；Day 24 backfill 具备 dry-run、目标库和行数双确认、活跃 writer 拒绝、批次上限、冲突预检、事务回滚、无数据丢失、重复执行和完成后 NULL 写入数据库护栏；环境执行证据、OIDC/RBAC/RLS 仍未完成；GAP-002 | 引入第二组织或账号边界 | 跨组织数据越界和错误操作 | Low | Critical | High | Platform Architect | Avoid | 阶段 2～3 | tenant escape、IDOR、环境 backfill、RLS 与后台上下文测试 | Mitigated | Day 25～30 完成剩余控制 |
