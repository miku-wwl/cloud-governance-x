...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RISK-0001 | 匿名管理与查询 API | Security | `Program.cs` 无认证授权；GAP-001 | API 被非本机或非预期主体访问 | 成本泄漏、云枚举、未授权 ETL 和写库 | Medium | Critical | Critical | Security Owner | Avoid | 阶段 2、8 | OIDC/RBAC/审计 E2E，匿名调用被拒绝 | Open | N/A |
| RISK-0002 | 无业务 tenant 隔离 | Security/Data | 三张核心表均无 `tenant_id`；GAP-002 | 引入第二组织或账号边界 | 跨组织数据越界和错误操作 | Medium | Critical | Critical | Platform Architect | Avoid | 阶段 2～3 | tenant escape、IDOR、RLS 与后台上下文测试 | Open | N/A |
| RISK-0003 | API/Worker 自动 migration | Reliability/Operations | 两个宿主启动时调用 `MigrateAsync`；GAP-003 | 多实例启动或高风险 schema 变更 | 迁移竞争、锁表、运行身份持有 DDL 权限 | Medium | High | High | Platform SRE | Mitigate | 阶段 1 | 独立 Migration Host、受限运行身份、升级演练 | Open | N/A |
