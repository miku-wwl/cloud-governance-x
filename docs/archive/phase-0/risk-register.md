...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RISK-0001 | 匿名管理与查询 API | Security | Day 26 已用真实 Entra token 验证 issuer、audience、scope、JWKS 和 Membership；Day 27 已建立 Membership role、tenant/CloudAccount/platform scope 和 RBAC 授权评估服务；Day 28 已为现有业务端点绑定 RBAC permission filter，health 保持 explicit anonymous；Day 29 已为授权成功/失败追加审计事件；Day 30 已补端点清单、匿名拒绝、RBAC deny、tenant escape 和审计 gate 测试；GAP-001 | 新增端点绕过授权、审计查询缺失或错误契约未硬化 | 成本泄漏、云枚举、未授权 ETL 和写库 | Low | Critical | High | Security Owner | Avoid | 阶段 2、8 | Day 30 Owner review、后续 API hardening 和生产运行门禁 | Mitigated | Day 30 Owner review |
| RISK-0002 | 无业务 tenant 隔离 | Security/Data | Day 23 已隔离新读写；Day 24 backfill 建立受控迁移和数据库护栏；Day 25 已验证 OIDC `iss/sub` 到 Membership/TenantContext 的真实请求管道；Day 27 已完成 RBAC 范围评估；Day 30 已补 tenant escape、query string tenant 伪造和 RBAC deny handler bypass gate；环境执行证据和 RLS 仍未完成；GAP-002 | 引入第二组织或账号边界 | 跨组织数据越界和错误操作 | Low | Critical | High | Platform Architect | Avoid | 阶段 2～3 | Day 30 Owner review、RLS 与环境 backfill 证据 | Mitigated | Day 30 Owner review |
...
| RISK-0018 | Azure CLI 用户身份作为运行身份 | Security/Operations | Day 26 已将本地 API caller 身份分离为无 secret 的 Entra public client；Azure Provider runtime 仍使用本地 `DefaultAzureCredential`；GAP-005 | 用户会话过期、订阅切换或权限过大 | 采集失败、越权和不可审计 Provider 身份使用 | High | High | Critical | Security Owner | Avoid | 阶段 5 | workload identity、最小 RBAC 与轮换演练 | Mitigated | Day 61 关闭 Provider runtime 身份缺口 |
