...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RISK-0001 | 匿名管理与查询 API | Security | Day 26 已用真实 Entra token 验证 issuer、audience、scope、JWKS 和 Membership；Day 27 已建立 Membership role、tenant/CloudAccount/platform scope 和 RBAC 授权评估服务；Day 28 已为现有业务端点绑定 RBAC permission filter，health 保持 explicit anonymous；GAP-001 | 新增端点绕过授权或授权失败不可审计 | 成本泄漏、云枚举、未授权 ETL 和写库 | Low | Critical | High | Security Owner | Avoid | 阶段 2、8 | Day 29～30 审计、端点清单复核和匿名拒绝 E2E | Mitigated | Day 30 gate 前复核 |
