...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RISK-0001 | 匿名管理与查询 API | Security | Day 26 已用真实 Entra token 验证 issuer、audience、scope、JWKS 和 Membership；Day 27 已建立 Membership role、tenant/CloudAccount/platform scope 和 RBAC 授权评估服务；现有业务端点尚未绑定授权 policy；GAP-001 | API 被非本机或非预期主体访问 | 成本泄漏、云枚举、未授权 ETL 和写库 | Medium | Critical | Critical | Security Owner | Avoid | 阶段 2、8 | Day 28～30 API policy、审计和匿名拒绝 E2E | Open | N/A |
