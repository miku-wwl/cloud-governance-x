...
| --- | --- | --- | --- | --- | --- | --- |
| GAP-001 | 管理 API 和查询 API 匿名 | Day 25 已接入标准 OIDC JWT Bearer 验证；Day 26 已用真实 Microsoft Entra delegated token、metadata/JWKS 和 Tenant Membership 完成 API 调用；Day 27 已建立 RBAC 权限与范围评估服务；Day 28 已为现有业务端点绑定 RBAC permission filter，health 保持 explicit anonymous；Day 29 已为授权成功/失败追加审计事件 | 当前业务端点匿名访问和授权不可追责风险已缓解；生产仍需要端点清单复核、rate limit、完整错误契约和 Phase 2 gate | 仅绑定本机并保持非公开 | 阶段 2、8 | Day 30 gate、后续 API hardening |
