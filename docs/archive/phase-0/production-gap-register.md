...
| --- | --- | --- | --- | --- | --- | --- |
| GAP-001 | 管理 API 和查询 API 匿名 | Day 25 已接入标准 OIDC JWT Bearer 验证；Day 26 已用真实 Microsoft Entra delegated token、metadata/JWKS 和 Tenant Membership 完成 API 调用；Day 27 已建立 RBAC 权限与范围评估服务；Day 28 已为现有业务端点绑定 RBAC permission filter，health 保持 explicit anonymous；Day 29 已为授权成功/失败追加审计事件；Day 30 已补 tenant escape、query string tenant、RBAC deny handler bypass 和 endpoint protection gate 测试 | 当前业务端点匿名访问和授权不可追责风险已缓解；生产仍需要 rate limit、完整错误契约、审计查询和生产运行门禁 | 仅绑定本机并保持非公开 | 阶段 2、8 | Day 30 Owner review、后续 API hardening |
| GAP-002 | 无业务 tenant 隔离 | Day 20～23 已建立 tenant 模型、可信上下文和 tenant-aware Repository；Day 24 backfill 增加数据库护栏；Day 25 已把验证后的 OIDC `iss/sub` 接入 Membership 和 TenantContext；Day 27 已建立 tenant/CloudAccount/platform scope RBAC；Day 30 已补 tenant escape 和 query string tenant 伪造 gate 测试 | 新写入、历史迁移、HTTP 身份转换、RBAC 范围评估和端点 gate 已有隔离机制，但各环境 backfill 和 RLS 尚未完成 | 仅受控开发 Tenant；生产不得运行 Development backfill | 阶段 2～3 | Day 30 Owner review、RLS 评估、环境 backfill 证据 |
...
| GAP-005 | Azure CLI 用户身份 | Azure Provider 仍使用 `DefaultAzureCredential` + 本地 `az login`；Day 26 的 API caller identity 已改用独立 Entra public client 和 delegated scope | Provider runtime 仍无 workload identity、轮换和最小权限证明 | 仅开发机使用 | 阶段 5 | Managed/Workload Identity、Provider RBAC |
...
| ID | 差距 | 当前证据 | 影响 | 目标阶段 |
| --- | --- | --- | --- | --- |
| GAP-015 | 测试层次不足 | Day 29 增加授权成功/失败审计 TestServer E2E、审计实体、DI 和模型配置测试；Day 28 增加端点级 authorization metadata、匿名 401、无 TenantContext 403、无权限 role 403 和 TestServer E2E identity→TenantContext→UseAuthorization→endpoint filter 测试；Day 27 已有 RBAC 矩阵和数据库 migration 回归 | 仍缺 Provider 故障注入、负载和生产规模回归 | 阶段 2～4、14 |
